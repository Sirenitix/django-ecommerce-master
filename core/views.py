...
def search_products(request):
    if request.method == "POST":
        ...
        if query_name:
            results = Item.objects.filter(title__contains=query_name).order_by('-price')
...
def search_category(request):
    if request.method == "POST":
        query_name = request.POST.get('title', None)
        parameter = " ".join(w.capitalize() for w in request.path.replace("/category/search/","").replace("_", " ").split())
        CATEGORIES_D = {v: k for k, v in CATEGORY_CHOICES}
        if query_name:
            results = Item.objects.filter(title__contains=query_name).filter(category__contains=CATEGORIES_D.get(parameter)).order_by('-price')
